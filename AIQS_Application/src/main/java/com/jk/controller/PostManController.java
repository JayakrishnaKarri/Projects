...
import com.jk.entity.Quote;
import com.jk.entity.UserProfile;
import com.jk.entity.Vehicle;
import com.jk.repository.CreditCardRepository;
import com.jk.repository.DriverRepository;
import com.jk.repository.InsuranceProviderRepository;
import com.jk.repository.QuoteRepository;
import com.jk.repository.UserProfileRepository;
import com.jk.repository.VehicleRepository;
...
public class PostManController {
	...
	public UserProfile getUserById(@PathVariable Integer userId) {
		return userRepo.findById(userId)
				.orElseThrow(() -> new RuntimeException("User is Not Found with the User Id: " + userId));
	}
	...
	public ResponseEntity<Object> deleteUserByAdmin(@PathVariable Integer userId) {
		...
		if (user.get().getRole().equals("ROLE_ADMIN"))
			return new ResponseEntity<>("Can't Delete The Admin", HttpStatus.BAD_REQUEST);

		Predicate<? super Driver> predicate = driver -> driver.getUserId().equals(user.get());
		Optional<Driver> driver = driverRepo.findAll().stream().filter(predicate).findFirst(); // Can User JPA
																								// implementation -
																								// findByDriverName from
																								// the DriverRepository
		if (driver.isPresent()) {
...
//			Iterable<Quote> q = quotes; 
			...
			/*
			 * quotes.forEach(q -> { q.setCreditCard(null); quoteRepo.save(q); });
			 */
		});
	...
	public ResponseEntity<Object> forgotPassword(@RequestBody UserProfile user) {
		UserProfile userFetched = userRepo.findByEmailIdAndUserName(user.getEmailId(), user.getUserName())
				.orElseThrow(() -> new RuntimeException("User Doesn't Exist"));
		if (!userFetched.getDOB().equals(user.getDOB())
				|| !userFetched.getContactNumber().equals(user.getContactNumber()))
			return new ResponseEntity<Object>("Re-enter correct user details to reset the password",
	...
	public ResponseEntity<Object> addQuote(@RequestBody Quote quote, @PathVariable Integer vehicleId,
			@PathVariable String insuranceProviderName, @PathVariable Long creditCardNumber) throws SQLException {
		...
		if (users.size() == 0)
			return new ResponseEntity<Object>(
					"Enter a Correct Credit Card Number That Belongs to the User: " + card.getUserId().getUserName(),
					HttpStatus.BAD_REQUEST);

		Vehicle vehicle = getVehicleById(vehicleId);
		if (vehicle.getQuote() != null)
			return new ResponseEntity<>(
					"Quote is Already Exists for this Vehicle: " + vehicle.getVehicleName() + ", " + vehicleId,
					HttpStatus.BAD_REQUEST);
		...
		try {
			...
			return new ResponseEntity<Object>(
					"Error While Updating the vehicleId: " + vehicleId + " for purchased Quote, quoteId: "
							+ savedQuote.getQuoteId() + "\n Re-Check the Vehicle Id & CreditCardNumber",
					HttpStatus.BAD_REQUEST);
		}
	...
	public Driver getDriverById(@PathVariable Integer licenseNumber) {
		return driverRepo.findById(licenseNumber)
				.orElseThrow(() -> new RuntimeException("Driver Doesn't Exists for License Number: " + licenseNumber));
	}
	...
	public ResponseEntity<Object> setVehicle(@PathVariable Integer licenseNumber,
	...
	public ResponseEntity<Object> saveDriver(@RequestBody Map<String, Object> map) {
		...
		if (!driverName.equals(user.getUserName()))
			return new ResponseEntity<Object>(
					"User Id: " + user.getUserId() + " doesn't matches with User Name: " + driverName,
					HttpStatus.BAD_REQUEST);
	...
	// Usually we don't change userId for a driver license number(User for a
	// Driver), but we can
	...
	public Vehicle getVehicleById(@PathVariable Integer vehicleId) {
		return vehicleRepo.findById(vehicleId)
				.orElseThrow(() -> new RuntimeException("Vehicle is Not Found for the Vehicle Id: " + vehicleId));
	}
	...
	public InsuranceProvider getInsuranceProviderById(@PathVariable String insuranceProviderName) {
		return insuranceRepo.findById(insuranceProviderName)
				.orElseThrow(() -> new RuntimeException(insuranceProviderName + " insurance provider is not found"));
	}
	...
	public CreditCard getCreditCardByNumber(@PathVariable Long creditCardNumber) {
		return creditRepo.findById(creditCardNumber).orElseThrow(
				() -> new RuntimeException("Credit Card is Not Found with the number: " + creditCardNumber));
	}
	...
	public ResponseEntity<Object> addCreditCard(@RequestBody CreditCard card, @PathVariable Integer userId) {
		if (card.getCreditCardNumber() < 1000000000)
			return new ResponseEntity<>("Incorrect Credit Card Number: " + card.getCreditCardNumber(),
					HttpStatus.NOT_FOUND);
		if (creditRepo.findById(card.getCreditCardNumber()).isPresent())
			return new ResponseEntity<Object>("Please select another Credit Card, Credit card: "
					+ card.getCreditCardNumber() + " is already exists", HttpStatus.BAD_REQUEST);
		UserProfile user = getUserById(userId);
		card.setUserId(user);
