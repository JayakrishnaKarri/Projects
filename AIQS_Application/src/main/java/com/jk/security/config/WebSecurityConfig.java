package com.jk.security.config;
...
public class WebSecurityConfig {
	...
	protected SecurityFilterChain filterChain(HttpSecurity http, HandlerMappingIntrospector introspector)
		...
		http.csrf(c -> c.disable());// (c -> c.ignoringRequestMatchers(mvcRequestMatcher.pattern("/"),
									// PathRequest.toH2Console()));
		http.headers(head -> head.frameOptions(frame -> frame.sameOrigin()));
		// .frameOptions().sameOrigin();
//		.and()
		http.authorizeHttpRequests(auth -> auth.requestMatchers(mvcRequestMatcher.pattern("/")).permitAll()
...
//			.and()
				.formLogin(log -> log.loginPage("/login").defaultSuccessUrl("/home").failureUrl("/login?error")
						.permitAll())
//				.loginPage("/login")
...
//					.permitAll()
//					.and()
				.rememberMe(rem -> rem.rememberMeCookieName("my-remember-me-cookie")
